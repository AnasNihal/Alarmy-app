...
export default function SetAlarmScreen() {
  ...
  return (
    <LinearGradient colors={['#F4F3FF', '#FFFFFF']} style={styles.safe}><SafeAreaView style={styles.safe}>
      <View style={styles.header}><Pressable onPress={() => router.back()}><Text style={styles.back}>‹ Back</Text></Pressable><Text style={styles.title}>Set alarm</Text><Pressable onPress={() => router.back()}><Text style={styles.cancel}>Cancel</Text></Pressable></View>
...
const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, header: { padding: 24, flexDirection: 'row', alignItems: 'center', gap: 22 }, back: { color: colors.primary, fontSize: 17 }, title: { color: colors.text, fontSize: 24, fontWeight: '700', flex: 1 }, cancel: { color: colors.primary, fontSize: 16, fontWeight: '600' }, content: { padding: 24 }, fieldLabel: { color: colors.text, fontSize: 15, fontWeight: '600', marginTop: 16, marginBottom: 9 }, timePicker: { backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1, borderColor: colors.border, alignItems: 'center', overflow: 'hidden' }, input: { backgroundColor: colors.surface, borderRadius: 14, padding: 16, color: colors.text, fontSize: 16, borderWidth: 1, borderColor: colors.border }, choices: { flexDirection: 'row', gap: 10 }, choice: { flex: 1, paddingVertical: 14, alignItems: 'center', borderRadius: 12, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, choiceSelected: { backgroundColor: colors.primary, borderColor: colors.primary }, choiceText: { color: colors.muted, textTransform: 'capitalize', fontWeight: '600' }, choiceTextSelected: { color: '#FFF' }, save: { marginTop: 34, borderRadius: 14, backgroundColor: colors.primary, padding: 17, alignItems: 'center' }, saveText: { color: '#FFF', fontSize: 17, fontWeight: '700' } });
